$(function () {
  ...
  function loadLoggedUser() {
    ...
    $(".logged-user-account-number").text(
      `${moment(payloadClaim(token, "account_number")).format("Do MMMM, YYYY")}`
    );
    $(".logged-user-account-officer").text(
      `${payloadClaim(token, "account_officer_first_name")} ${payloadClaim(
        token,
        "account_officer_last_name"
      )}`
    );
  ...
  function loadDashboard() {
    ...
    $.ajax({
      type: "GET",
      url: `${API_URL_ROOT}/dashboards?call=customer_dashboard&token=${token}`,
      dataType: "json",
      ...
      success: function (response) {
        if (response.error == false) {
          ...
          $(".total-credits").text(formatCurrency(dashboard.total_credits));
          $(".total-debits").text(formatCurrency(dashboard.total_debits));
        } else {
          showSimpleMessage("Attention", response.message, "error");
        }
      },
  ...
  function newFundsTransfer() {
    Swal.fire({
      ...
      if (result.value) {
        ...
        if (isNaN(amount)) {
          unblockUI();
          showSimpleMessage("Attention", "Amount must be a number", "error");
          return false;
        }
      ...
      if (selectedOption === "Safe Lock") {
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            if (response.error === true && response.message === "LOCK_ACTIVE") {
              showSimpleMessage(
                "Attention",
          ...
          error: function (response) {
            unblockUI();
            showSimpleMessage("Attention", "An error ocured", "error");
          },
        });
      }
    });
